...
- Pointer-reactive spotlight, tilt, magnetic, cursor-follow, reveal-mask, and
  pointer-parallax effects are selectable on arbitrary elements with explicit
...
- ADR 0066 added Variant presets for forms, carousels, accordions, and tabs;
  ...
  `cursor-follow`, `reveal-mask`, and `pointer-parallax`, but only Form
...
- **June 2026 update:** Pointer FX is now owner-facing beyond Form
  ...
  `magnetic`, `cursor-follow`, `reveal-mask`, or `pointer-parallax`, explicit
  reduced-motion mode, validator/Yjs persistence, renderer metadata, editor
  inspector controls, and named runtime failure events.
...
- Pointer-fx exists as a runtime concept with `spotlight`, `tilt`, `magnetic`,
  `cursor-follow`, `reveal-mask`, and `pointer-parallax`.
...
- **June 2026 update:** Pointer FX now has an element-level inspector catalog.
  ...
  `reveal-mask`, or `pointer-parallax` to arbitrary elements; Form `spotlight`
  also emits explicit reduced-motion metadata; malformed runtime attributes
  fail through `opencanvas:pointer-fx-failure`.

User-visible miss:

- Cannot reproduce cursor trails, image-follow cursors, drag/inertia sliders,
  or hover-to-preview grids.

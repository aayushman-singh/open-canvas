...
- **Decision 4 (PARTIAL 2026-05-30):** `assertFiniteNumber` / `assertOptionalFiniteNumber` / `assertNonEmptyString` / `assertOptionalNonEmptyString` / `assertOptionalBoolean` shipped; clearest call clusters swept through them (validateBox, validateMotion, validateActionHref href/pageId, validatePage title/author/tags/category/description/canonical/noIndex, VideoMediaElement playback flags, SITE_FIELD_VALIDATORS booleans). Remaining: `assertUnique` helper for the duplicate-id pushes; `validateTextContent` path-shape refactor to drop the "text element <id>." prose prefix.

...
**Verified-state:** re-verified 2026-06-03 after Phase 1 + Phase 2a–g landed (commits `d2dcf76..f0cf1bb`) — **build infrastructure is as-built; source migration is ~5%; editor route, asset binding, and shell rewrite are deferred to Phase 3.**
- Decision 1 (module tree + bundle delivery): **partial.** [`src/editor-client/`](../../src/editor-client/) exists with 18 modules (15 extracted units + `index.ts` + `tsconfig.json` + `types.d.ts` + `styles.css`). Pure-leaf extraction is exhausted; every remaining chunk in [`src/editor/canvas-client.ts`](../../src/editor/canvas-client.ts) reads or mutates IIFE-local state and requires a context-object design before continuing. The editor route still serves from the template-literal `canvasClientScript()` — the new module tree is dead code until Phase 3 cutover.
- Decision 2 (Bun.build script + manifest module): **as-built.** [`scripts/build-editor-client.ts`](../../scripts/build-editor-client.ts) (96 LOC) is wired to `editor-client:build`; it emits `dist/_assets/index-<hash>.{js,css}` and writes [`src/_assets/manifest.generated.ts`](../../src/_assets/manifest.generated.ts) (`EDITOR_CLIENT_MANIFEST`) on every run.
- Decision 3 (content-hash filenames + `Cache-Control: immutable`): **partial.** `naming: '[name]-[hash].[ext]'` is wired; cache headers are deferred because no consumer fetches the bundle yet (no Wrangler `[assets]` binding).
- Decision 4 (source maps in dev, not prod): **not as-built.** `scripts/build-editor-client.ts` unconditionally sets `sourcemap: 'none'`; the dev/prod split named by this decision is not implemented and no dev-build path exists yet.
- Decision 5 (scope confined to `canvas-client.ts` + `canvas-styles.ts`): **as-built.** Extraction has not touched `public-styles.ts`, `interactive/build.ts`, dashboards, or inline route scripts.
- Decision 6 (minimal HTML shell + `window.__rev01EditorBoot`): **not as-built.** The editor route still emits a `canvasClientScript()` template literal inline; no shell, no boot global.
- Decision 7 (Wrangler `[assets]` binding scoped to `/_assets/`): **not as-built.** [`wrangler.toml`](../../wrangler.toml) has no `[assets]` block; the `binding = "ASSETS_BUCKET"` at line 91 is the unrelated R2 bucket from ADR 0006.
- Decision 8 (ADR 0014 supersession + plugin deletion): **partial.** [ADR 0014](0014-template-literal-data-substitution.md) flipped to Rejected on 2026-06-03 — vacuously, since the substitution plugin was never written and there was nothing to delete. The twelve mirrored constants in [`src/editor/canvas-client.ts`](../../src/editor/canvas-client.ts) persist because the file is still the production source; they collapse to normal `import` statements only on Phase 3 cutover.
- Current line counts: [`src/editor/canvas-client.ts`](../../src/editor/canvas-client.ts) is **14,372 lines** (Context cites 9,333; the inlined source has grown ~5k since this ADR was drafted on 2026-05-29). [`src/editor/canvas-styles.ts`](../../src/editor/canvas-styles.ts) is **3,733 lines** (Context cites 2,089). The growth between 2026-05-29 and 2026-06-03 reinforces rather than weakens the ADR's premise — every week of deferral concentrates more cost on the inline-delivery model.

Status stays **Proposed** because the editor route still serves from the template literal — no `Accepted-context:` proof can be written until Phase 3 cuts the route over to the manifest URL. The decisions remain the intended target. The ADR flips to Accepted only when all eight decisions are as-built and this `Verified-state:` block is replaced by an `Accepted-context:` block citing the landed mechanisms.

...
2. **The IIFE body is lifted into a real TS function `createEditor(boot: EditorBoot): void` in [`src/editor-client/index.ts`](../../src/editor-client/index.ts). The function caches DOM refs, fetches initial state, constructs the `EditorContext` object as a local variable, wires event listeners that close over `ctx`, and dispatches into extracted modules with `(ctx, …)` signatures.**

   **Why:** `createEditor` IS today's IIFE, lifted out of the template literal into a TS module. No paradigm shift, no new construct introduced. Each Phase 2h+ extraction moves a chunk of inline code out of `createEditor`'s body into a sibling module that takes `ctx` as a parameter; `createEditor` shrinks each phase until, by Phase 3, it's just the boot sequence (DOM caching → initial state fetch → event-listener wiring → `await loadInitialState(ctx)`).
...
3. **Extracted modules consume `ctx` as the first parameter of plain function exports. No classes, no factories, no DI containers.**
   ...
   **Why:** the function-with-ctx pattern matches the IIFE-closure shape exactly. A reviewer reading `dragEnd(ctx, event)` sees the same function body the IIFE has today with `ctx.` prefixes. Classes would force a `new` site, method-binding decisions, and an instance-vs-static call decision per helper — three places where the diff stops being mechanical. DI containers are overkill for a single-instance editor whose dependencies are all known at build time.
   ...
   This would be wrong if other parts of the codebase were class-oriented and the editor's plain-function shape stood out. The codebase is uniformly function-oriented ([`src/canvas/elements/`](../../src/canvas/elements/), [`src/agent/`](../../src/agent/), [`src/canvas/`](../../src/canvas/) — none use classes for orchestration). Plain functions fit.
...
- **No isolation testing during the migration.** Each extracted module is exercised through the existing editor smokes plus, where feasible, a parity smoke comparing IIFE inline output to extracted module output for the same input. Pure unit tests against extracted modules with a mocked `ctx` are deferred — they fit naturally with the post-Phase-3 decomposed contexts and are awkward against the wide migration shape.
...
- **Land empty `EditorContext` interface + `createEditor` factory stub** in [`src/editor-client/editor-context.ts`](../../src/editor-client/editor-context.ts) and [`src/editor-client/index.ts`](../../src/editor-client/index.ts). No behavioural change — the editor route still serves `canvasClientScript()`. This commit gates the design without exercising it.
...
- **Per-phase parity smoke** where feasible: hand the same input to the inline IIFE path and the extracted module, assert byte-identical output. Some phases (drag handlers, event-driven flows) won't admit a clean parity smoke; for those, document the behavioural assertion the existing editor smoke must continue to satisfy.

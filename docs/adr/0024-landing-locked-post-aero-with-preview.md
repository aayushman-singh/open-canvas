...
**Accepted-context:** verified 2026-06-01 — design lock and static-at-request decisions match the code at `src/landing/index.tsx`; `src/landing/PREVIEW.html` is checked in and `bun run landing:preview` regenerates from `src/landing/build-preview.ts`. Two real gaps were discovered during verification and are recorded in Follow-ups: (1) `landing:preview` cannot regenerate without `CLERK_*` env present because the auth-state header swap (commit `73711fd`) added a `resolveClerkKeys(c.env)` call on the render path; (2) the checked-in `PREVIEW.html` has drifted from the current runtime render. Acceptance flips on the *decisions*; the preview-mechanism integrity is named as a deliberate follow-up rather than blocking the lift.
...
3. **A byte-identical `PREVIEW.html` is checked in at `src/landing/PREVIEW.html` and regenerated via `bun run landing:preview`.** The preview is the same output the route produces at request time.
   ...
   **Why:** the landing page is visually-heavy and design-iterative; reviewers and the maintainer review changes by *seeing* the page, not by reading the JSX. Booting Wrangler to render once is overhead a designer or a casual reviewer should not pay. A checked-in HTML artifact makes the page inspectable with one double-click. The preview is regenerated by the same Hono `app.request('/', ...)` path the worker takes, so the artifact is *intended* to be byte-identical to the runtime render. Enforcement is mechanical when a `landing:preview:check` script exists; today the comparison is by hand (or by `git diff src/landing/PREVIEW.html` in PR review). A mechanical assertion is named in Follow-ups, complicated by the route's current dependency on per-environment `CLERK_*` keys.
...
- **DONE (pre-2026-06-01)** — `src/landing/SUBSYSTEM.md` deleted.
- **OPEN — preview-mechanism drift.** Two gaps surfaced during the 2026-06-01 acceptance audit:
  1. `bun run landing:preview` cannot regenerate `PREVIEW.html` without `CLERK_PUBLISHABLE_KEY` / `CLERK_SECRET_KEY` (or their `_TEST` siblings) in env, because the auth-state header swap (commit `73711fd`) put a `resolveClerkKeys(c.env)` call on the landing render path. Without env, the route 5xx's and `PREVIEW.html` becomes `Internal Server Error`. The Clerk publishable key is also embedded in the rendered HTML, so different developers' `.env` choices produce different bytes — byte-identity is no longer trivially deterministic across machines.
  2. The currently checked-in `src/landing/PREVIEW.html` (1361 lines) is stale relative to the runtime render — the auth-header swap landed without regenerating the artifact.
  Closing this is a real piece of work: either (a) decouple the landing render from Clerk env (render the static signed-out chrome with no Clerk call; only consult Clerk when a session cookie is present), (b) make `build-preview.ts` use a documented stub Clerk PK that survives `clerkFrontendApiHost` parsing, or (c) accept the determinism loss and require a fixed env in CI. Until one is chosen, `PREVIEW.html` cannot be trusted as a byte-identical mirror — `git log src/landing/PREVIEW.html` is still a useful design changelog but not an integrity gate.
- A `bun run landing:preview:check` script that exits non-zero on drift is the natural mechanical assertion the ADR's Consequences line describes, blocked on resolving the gap above.

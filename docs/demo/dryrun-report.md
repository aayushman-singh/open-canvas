...
| S2.B.1 | Editor "Add" sidebar has **14** direct-add buttons (one per element type) | Sidebar shows **Text / Image / Video / Button / Shape / Container / Chart / Form / Embed / Code / Accordion / Carousel / Table / Nav** — 14 buttons but Media is split into Image+Video, and **Collection is not in the sidebar Add list at all** | Script Collection as a Sections-tab recipe, not a direct-add button. The Act 1 script now keeps Collection in the recipe-backed flow. |
...
5. **Keep Collection scripted as a recipe-backed section** unless product direction changes to add a direct-add button and dedicated inspector.

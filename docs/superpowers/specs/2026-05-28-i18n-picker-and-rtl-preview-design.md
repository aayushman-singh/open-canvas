...
| `state.defaultLocale` schema field already exists (`CanvasSiteState.defaultLocale?: string` at schema.ts:400). PATCH route must add it without breaking existing PATCHes. | Additive optional key on the PATCH body; the existing handler ignores unknown keys today, so old clients keep working. |

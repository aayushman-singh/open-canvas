...
| B-AUTH-002 | Sign out (dashboard kebab in shell or Profile page) | `a.dash-sign-out` (index) / `a.btn-signout` (profile) | link | `GET https://accounts.<root>/sign-out?redirect_url=...` | Clerk portal terminates the session and 302s back to landing; cookies cleared | `src/routes/dashboard/index.tsx:1236`, `src/routes/dashboard/profile.tsx` (sign-out button) | signed in |
...
| B-PUB-002 | edit this site | footer `a[href="/__edit"]` | link | opens `/api/on-site-edit` (Clerk popup → sets `__rev01_edit` cookie) | popup auths owner; on close, /__edit reopens editor on parent site | `src/auth/edit-token.ts` + `src/routes/api/on-site-edit.ts` | branding enabled |
...
| B-SST-012 | Remove (per row) | `.remove-btn` (per collaborator row) | button | `DELETE /api/sites/:id/collaborators/:collabId` | 200; row removed | 270-284 | ⚠️ N8: button currently fires no network — verify wiring |
...
| B-SNP-002 | Restore (per row) | `button[data-timeline-action="restore"]` | button | opens `__rev01Modal.confirm`; on OK → `POST /api/sites/:id/snapshots/:snapshotId/restore` | confirm modal (uses `.r-modal-backdrop`!) → 200 → page reloads after 800ms; current canvas == restored snapshot | 328-330 | snapshot exists |
...
| B-ED-INS-042 | × (clear asset) | inspector `button.style-btn-clear` | button | clears `assetId` (re-points to `__placeholder__`) | preview reverts to placeholder ⚠️ N9: doesn't fully clear `elementStyle` |
...
There are **two separate modal systems** in this codebase. Agents need to know which prefix to query.
...
| B-MOD-SHELL | Dashboard shell modal manager | `.r-modal-backdrop` + `.r-modal` | `/dashboard/sites/:id/snapshots` Restore + Preview confirms, presumably other dashboard pages | `src/routes/dashboard/shell.tsx:180`, `:378-379` |

⚠️ **N10**: this dual prefix is a real footgun. If you write a generic "dismiss any open modal" helper, query both: `document.querySelector('.rev01-modal-backdrop, .r-modal-backdrop')`.
...
- **A2 — Dashboard list pages**: B-DASH-*, B-TPL-*, B-SHOP-*, B-SET-* (currently blocked on B1/N1, so this agent's first job is to verify the dashboard 500 is fixed before continuing).
- **A3 — Site-management sweep (per site `:id`)**: B-SHELL-101..110, B-SST-*, B-NAV-*, B-FRM-*, B-SNP-*, B-DOM-*, B-SADD-*, B-A11Y-*, B-CHT-*.
- **A4 — Editor topbar + tabs + Add panel**: B-ED-T-*, B-ED-TAB-*, B-ED-ADD-*, B-ED-PG-*.
...
- **A6 — Editor chat + viewport + page-settings + symbols**: B-ED-CHAT-*, B-ED-VPT-*, B-ED-PSET-*, B-ED-SYM-*.

 ...
 * picker writes the chosen font's family chain directly into a text
 * element's `pinnedStyle["font-family"]` (so the renderer-side `font:<hash>`
 * resolver never sees it). The public renderer therefore needs every
 * uploaded font's face declaration available on the page so element-level
 * pins can resolve to the WOFF2 bytes the Owner uploaded.
 *

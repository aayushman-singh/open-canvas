...
//
// This check is *guarded*: it runs only when the FormElement type is
// registered. `'form'` is registered in `ELEMENT_TYPES` so the guard is
// effectively always satisfied today; the guard exists so the audit
// subsystem stays self-contained if a future regression removes the form
// ElementType, rather than crashing here.
//
...
// canvas element registry; if the file moves or is removed, this import

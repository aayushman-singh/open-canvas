...
//
// ADR 0058 Phase 2k.b — chat session form + SSE streaming + suggestion
// chips + chat-message helpers. canvas-client.ts:14084-14386 carries the
// inline twin (chat session init block nested inside the boot-time
// try-catch inside the main IIFE). The twin retires on ADR 0015 Phase 3
// atomic cutover; until then, the inline IIFE is the production source-
// of-truth and this module is dead code.
//
// Three exports:
//
...
//     handler), and attaches the form submit listener that POSTs to
//     /sites/<id>/chat and SSE-streams the response. The submit listener
//     keeps the inner declarations (ensureAssistantBubble, readChunk,
//     removeThinking) the inline twin uses — they capture the in-flight
//     state per turn, so lifting them to module-level would require
//     passing 4-5 parameters per call.
//
...
// stay inline in canvas-client.ts (line 10783-10812+ and beyond). Phase
// 2k.b's extracted SSE branch invokes them via ctx forward declarations
// so the module typechecks while Phase 2n still owns the implementation.
//

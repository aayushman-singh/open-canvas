...
export function mountTextFontFamily(
  ...
  async function handleDelete(font: EditorCustomFont): Promise<void> {
    ...
    // Call `confirm` on the modalApi receiver (not via an extracted
    // reference) so the unbound-method lint stays happy — `confirm` is a
    // non-arrow method on `OpencanvasModalGlobal`.
    const confirmed = await modalApi.confirm(
      `"${font.name}" will be removed from this site. Any element still pointing at it will fall back to the style-kit default.`,
      {
        title: 'Delete font?',
        confirmLabel: 'Delete',
        danger: true,
...
// `window.__opencanvasModal` is declared globally by editor-client/index.ts
// as `OpencanvasModalGlobal` (see editor-client/modals.ts). The previous
// local `declare global` in this file used the older
// `confirm(opts: { ... })` signature and collided with the canonical
// `confirm(msg, opts)` shape — removed during the PR #31 rebase fix.

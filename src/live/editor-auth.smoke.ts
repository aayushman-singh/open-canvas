...
assert(
  canUseLiveEditorSocket(
    { siteCustomerId: 'owner-customer', collaboratorCustomerId: null },
    'owner-customer',
  ),
...
const env = {
  // Encodes live-auth-smoke.clerk.accounts.dev$; buildOnSiteEditorOptions
  // resolves clerk-js from the publishable key when no env override is set.
  CLERK_PUBLISHABLE_KEY: 'pk_test_bGl2ZS1hdXRoLXNtb2tlLmNsZXJrLmFjY291bnRzLmRldiQ=',
...
assert(
  typeof opts.wsToken === 'string' && opts.wsToken.length > 0,
  'expected public editor wsToken',
);

const wsPayload = await verifyEditToken(opts.wsToken, env.UNLOCK_SIGNING_SECRET);
assert(
  wsPayload?.siteId === 'site-live-auth',
  'expected wsToken siteId to match public editor site',
);
assert(
